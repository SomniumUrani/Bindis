...
void * keysManager(void * arguments){
	...
	char c;
	while (!(*args->keys & BD_KEY_Q)){
		*args->keys = 0;
		c = 0;
		read(STDIN_FILENO, &c, 1);

		if 		(c == args->binds->keyQ) *args->keys |= BD_KEY_Q;
		else if (c == args->binds->keyA) *args->keys |= BD_KEY_A;
		else if (c == args->binds->keyB) *args->keys |= BD_KEY_B;
		else if (c == args->binds->keyC) *args->keys |= BD_KEY_C;
		else if (c == args->binds->keyD) *args->keys |= BD_KEY_D;
		else if (c == args->binds->keyE) *args->keys |= BD_KEY_E;
		else if (c == args->binds->keyF) *args->keys |= BD_KEY_F;
		else if (c == args->binds->keyG) *args->keys |= BD_KEY_G;
